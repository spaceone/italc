...
void ToolButton::enterEvent( QEvent * _e )
{
	...
	if(
#ifdef ITALC3
		!MasterCore::personalConfig->noToolTips()
#else
		!s_toolTipsDisabled
#endif
		&& !m_title.isEmpty() && !m_descr.isEmpty() )
	{
...
#ifdef ITALC3
	if( MasterCore::personalConfig->toolButtonIconOnlyMode() )
#else
	if( s_iconOnlyMode )
#endif
	{
...
#ifdef ITALC3
	if( MasterCore::personalConfig->toolButtonIconOnlyMode() == false )
#else
